...
public class Magasin {
    ...
    private Produit[] ensprod = new Produit[2];
    ...
    public void ajouterProduit(Produit q) throws MagasinPleinException,PrixNegatifException {
        try {
            if (chercher(q) == true) {
                System.out.println("Produit Existe !");
            } else {
                if (q.getPrix() < 0) {
                    PrixNegatifException PrixNegatifException = new PrixNegatifException();
                }
                this.nbrProduits++;
		...
		MagasinPleinException magasinPleinException = new MagasinPleinException();
		}
