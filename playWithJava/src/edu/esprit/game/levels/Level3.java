...
public class Level3 {
	...
	String names = employees.stream().map((t) -> t.getName()).reduce("",(t1,t2)->t1 + " " + t2); 
						
	/* TO DO 2: Retourner une chaine de caract�re qui contient tous les noms des employ�s en majuscule separ�s par # */	
	String namesMajSplit = employees.stream().map((t) -> t.getName()).reduce("",(t1,t2)->t1 + "" + t2).toUpperCase() ; 
	...
	TreeSet<Employee> emps2 =  employees.stream().collect(Collectors.toCollection(() -> new TreeSet<> (  (t1, t2) -> t1.getName().compareTo(t2.getName()))));
	...
	Map<Integer, String> mm = employees.stream().collect(Collectors.toMap());                
   

	/* TO DO 7: Retourner le  min, max,average, sum,count des salaires */
	String s = employees.stream().mapToInt((t) -> t.getSalary()).summaryStatistics().toString() ;
